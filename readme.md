...
Now, let's set up the ssh lockdown.
First, lets figure out what command the sync script runs.

On the remote, prepend the following to your key:
command="/home/bigdog/.ssh/log-cmds.sh" ssh-ed25519 AA....
And create the log-cmds script. in it, put:

    #!/bin/bash
    echo "$(date -Iseconds) $SSH_ORIGINAL_COMMAND" >> /home/bigdog/.ssh/commands.log
...
this to your authorized_keys:
command="/home/bigdog/.ssh/rm-viewer-sync-allowlist.sh",no-pty,no-x11-forwarding,no-agent-forwarding,no-port-forwarding,no-user-rc ssh-ed25519 AA....
...
If you use rm-uploader (https://github.com/avncharlie/rm-uploader, a web ui to
